...
#    # np.savetxt("q3_quantile.csv", percentile, delimiter=",")
    ...
    for i in range(11):
        ...
        data0 = np.squeeze(np.asarray(beta_store[:,i]))
    ...
    # Now plot    
        ...
        axarr[0].plot(data0)
        tt_1='Traceplot of beta_'+str(i)
        axarr[0].set_title(tt_1)
        axarr[1].plot(dist_space0, kde0(dist_space0) )
        tt_2='Posterior density of beta__'+str(i)
        axarr[1].set_title(tt_2)
        filename = 'Beta_'+str(i)+'.pdf'
        f.savefig(filename)
...
bayes_logreg(y,x,beta_0,sigma_0_inv,niter=10000,burnin=1000, \
                 print_every=1000,retune=100,verbose=True) 
